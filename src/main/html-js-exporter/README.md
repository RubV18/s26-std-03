...
The exporter is designed to work together with the Loader component included in this repository.
...
The exporter never reads the original Builder model directly. Instead, it consumes only the Intermediate Representation (IR) generated by the Loader.

This separation allows the exporter to remain product-independent while supporting any product that follows the Loader IR schema.
...
- data/
    nespresso.json
    Air_fryer_Philips_HD9252.jsons
